...
- **Measurement gate executed:** Collected 10 interactive samples (Ctrl+R capture + Codex send) and recorded results in `LATENCY_MEASUREMENTS.md`. Findings: voice pipeline averages 1.19 s, Codex averages 9.2 s (≈88 % of total latency), so Phase 2B work remains blocked until Codex latency is addressed or stakeholders approve moving forward despite limited ROI.
- **Persistent PTY fix:** Reworked the Codex backend health check so it waits up to 5 s, verifies the child process is alive, and no longer injects test prompts. Python helper path removed; persistent sessions now remain responsive and subsequent prompts reuse the PTY without CLI spawn overhead. Future work is to stream Codex output so the UI reflects token-by-token thinking.
- **Codex latency plan drafted:** Added `CODEX_LATENCY_PLAN.md` outlining telemetry tasks, PTY/CLI health checks, CLI profiling, alternative backend options, and success criteria. Phase 2B remains gated on completing this plan and making a go/no-go decision.
- **Measurement Gate Implementation**: Delivered `latency_measurement` binary (`rust_tui/src/bin/latency_measurement.rs`) + wrapper script (`scripts/measure_latency.sh`) that instruments full voice→Codex pipeline with REAL `Instant::now()` timing (NO estimates, NO TODOs). Parses actual timing logs from voice.rs when `log_timings=true`, provides bottleneck analysis with recommendations (proceed/defer/hybrid based on Voice% vs Codex%). Created `TIMING_IMPLEMENTATION.md` documenting all measurement sources and proving zero fake data. Next: collect 10+ samples and decide Phase 2B path based on data.
